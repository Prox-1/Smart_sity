...
from utils import q_learning, sumo_utils, test_reward
...
sumoCmd = [sumoBinary, "-c", sumoConfig, "--no-warnings",
           ...
           "--log", os.path.join(output_base_dir, "sumo.log"),
           "--error-log", os.path.join(output_base_dir, "sumo_errors.log"),
           "--verbose", "true"]
...
ACCIDENT_MAX_CONCURRENT = 2    # одновременно активных аварий
...
try:
    ...
    for episode in tqdm(range(NUM_EPISODES), desc="Episodes"):
        ...
        for current_step in range(MAX_SIMULATION_STEPS):
            ...
            global_reward = test_reward.calculate_global_reward(
                tls_ids,
                controlled_edges_dict,
                unique_edges_count,
                metrics=metrics_cache,   # <— новый параметр
            )

            # Обновления по каждому светофору
            for tls_id in tls_ids:
                # Локальная награда: обязательно используем controlled_edges_dict[tls_id]
                local_reward = test_reward.calculate_local_reward(
